...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'connect nodes, inspect the current graph, or export the canvas as a PNG. '
      + 'Units matter: serialized x/y (getData/setData) are GRID units where 1 unit = 20px, while node w/h and '
      + 'the x/y accepted by weave.addNode are PIXELS. A default node is 170x80px (~8.5x4 grid cells); place '
      + 'neighbouring nodes ~15 grid units apart horizontally and ~10 vertically so node edges leave ~6 empty '
      + 'cells — a normal graph spans tens of grid units, so coordinates beyond +-100 mean you confused grid '
      + 'units with pixels.',
    parameters: {
      code: {
        type: 'string',
